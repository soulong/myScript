...
read_plate_info_1 <- function(file, add_directory=F) {
  ...
  plate_info <- map(sheets, \(x) readxl::read_xlsx(
    ...
    bind_cols() %>%
    rename_with(~ str_c("Metadata_", .x), everything()) %>%
    bind_cols(Metadata_well = 
                map2_chr(rep(LETTERS[1:16], 24), 
                         rep(1:24, each = 16), # %>% str_pad(2, pad = "0"), 
                         ~ str_c(.x, .y)), .) %>% 
    filter(!if_all(!Metadata_well, is.na)) # remove all empty rows
  
  if(add_directory) {
    plate_info <- plate_info %>%
      mutate(Metadata_directory=file %>% 

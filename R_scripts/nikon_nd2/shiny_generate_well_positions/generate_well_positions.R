...
if(F) {
  ...
  well_fields_filtered <- all_well_fields %>% 
    # filter(str_detect(well, "^[BCDEF]")) %>% 
    # filter(!str_detect(well, "^[A-Z](1)$")) %>% # remove col 1
    # filter(!str_detect(well, "^[A-Z](10)$")) %>% # remove col 1
    # filter(!str_detect(well, "^[A-Z](11)$")) %>% # remove col 1
    filter(!str_detect(well, "^[A-Z](12)$")) %>% # remove col 12
    filter(!str_detect(well, "^[A-Z](23)$")) %>% # remove col 12
    filter(!str_detect(well, "^[A-Z](24)$")) %>% # remove col 12
    filter(!str_detect(well, "^[A]")) %>% # remove row A
    # filter(!str_detect(well, "^[G]")) %>% # remove row A
    filter(!str_detect(well, "^[H]")) %>% # %>%  # remove row H
    filter(!str_detect(well, "^[P]"))# %>%  # remove row H
  # filter(!str_detect(well, "[A-Z][1-6]$")) %>%
    # filter(!str_detect(well, "[A-Z](11)|(12)$")) %>%
    # filter(str_detect(well, "^[BCDE]"))
}
...
save_xml(final, 
         str_glue('{normalizePath("~/../Desktop")}/{Sys.Date()}_{fname}.xml'))

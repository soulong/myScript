...
for(use_db in c(#"hist_100_60000_quantile_50_logspace",
  ...
  p1 <- hist_filter %>% 
    mutate(bin_prop=bin_count/sum(bin_count), .by=c(directory, well, field, cell_id)) %>% 
    arrange(celltype) %>% 
    split(., .[['directory']]) %>% 
    imap(\(x, y) x %>% 
           ggplot(aes(x=bin_edge_mean, y=bin_prop, color=group, fill=group)) +
           geom_smooth() +
           labs(x="Intensity Bins", y="Pixel Proportion of Each Cell", title=y) +
           theme_bw() +
           theme(panel.grid=element_blank())
    ) %>% 
    patchwork::wrap_plots(ncol=3)
  # print(p1)
  ggsave(str_glue("{Sys.Date()}_histogram_{use_db}.pdf"), 
         p1, width=12, heigh=6)
  ...
  p2 <- hist_filter %>% 
    reframe(bin_mean=mean(bin_mean),
            ...
            .by=c(directory, group, celltype, bin_edge_mean)) %>% 
    group_by(directory, group, celltype) %>%
    # Ensure bins are in order
    arrange(bin_edge_mean, .by_group=T) %>%  
    ...
    split(., .[['directory']]) %>% 
    imap(\(x, y) x %>% 
           ggplot(aes(bin_edge_mean, cdf, color=group)) +

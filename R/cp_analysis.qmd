...
c("read_plate_info.R", 
  "cp_analysis_helper.R", 
  "ggplot_helper.R",
  "fit_ic50.R") %>% 
  file.path(paste0(Sys.getenv('USERPROFILE'),
                   "/Documents/GitHub/myScript/R"), .) %>%
  walk(source)
```

## read cellprofiler db
```{r}
result_db_files <- list.files(".", recursive=T, full.names=T) %>% 
  str_subset("cp_result.db") %>% 
  str_subset("jobs", negate=T) %>% 
  set_names(., nm=map_chr(., \(x) str_split_1(x, "__.*-Measurement") %>% 
                            .[1] %>% basename)) %>% 
  print()
rawList <- result_db_files %>% 
  map(\(x) read_sqlite(
    x, 
    remove_object_prefix=T,
    subset_areashape_category=c("AreaShape_Area","_EquivalentDiameter","_Eccentricity"), 
    subset_intenstiy_category=NULL, # c("IntegratedIntensity_", "MeanIntensity_")
...
if(file.exists(metadata_file)) {
  ...
  channel_names=c("GFP"="ch0")
}

# channel_names=NULL
if(!is.null(channel_names)) {
  for(idx in seq_along(channel_names)) {
    for(raw_name in names(rawList)) {
      # rename for raw$image
      rawList[[raw_name]]$image <- rawList[[raw_name]]$image %>% 
        rename_with(\(x) str_replace(
          x, channel_names[idx], names(channel_names)[idx]),
          !starts_with("Metadata"))
      
      # rename for raw$object
      for(obj in names(rawList[[raw_name]]$objects)) {
        rawList[[raw_name]]$objects[[obj]] <- rawList[[raw_name]]$objects[[obj]] %>% 
          rename_with(\(x) str_replace(
            x, channel_names[idx], names(channel_names)[idx]))
      }
      
      # rename for raw$object variable names
      names(rawList[[raw_name]]$objects) <- names(rawList[[raw_name]]$objects) %>% 
        str_replace(channel_names[idx], names(channel_names)[idx])
    }
  }
}
```
...
## metadata plate info
```{r}
plate_info_files <- list.files(".", "plate_info.xlsx", recursive=T, full.names=T) %>% 
  set_names(., nm=map_chr(., \(x) str_split_1(x, "__.*-Measurement") %>% 
                            .[1] %>% basename)) %>% 
  print()

plate_info <- plate_info_files %>%
  map(read_plate_info, colname_prefix='Metadata_') %>%
  list_rbind(names_to="Metadata_directory") %>% 
...
if(T) {
  ts <- readxl::read_excel("D:\\LGLab\\Project_TP53\\p53_TS.xlsx") %>% 
    filter(!row_number() %in% c(1:2)) %>% #glimpse()
    dplyr::select(2,4,10,11,14) %>% #glimpse()
    dplyr::rename(mutant=1,freq=2,ts_ratio=3,id_hh=4,id_lh=5)
  
  plate_info <- plate_info %>% 
    left_join(ts, by=join_by(Metadata_vector==id_lh)) %>% #view()
    mutate(mutant=str_replace_na(mutant, "#")) %>% 
    mutate(Metadata_vector=str_c(Metadata_vector, mutant, sep="_"))
}
```
...
if(F) {
  image <- image %>%
    ...
    #   print()
    mutate(Metadata_batch=ifelse(
      str_detect(Metadata_directory, "2025-11-08"), "2025-11-08", NA)) %>% 
    glimpse()
  count(image, Metadata_batch, Metadata_temp)
}
...
image <- image %>% 
  { if(length(unique(image[["Metadata_stack"]]))==1 ) {
    select(., !Metadata_stack) } else .} %>% 
...
## cell structure
```{r}
# glimpse(rawList[[1]]$objects)
structures <- c("cell", "nuclei", "membrane", "plasma")
structures <- intersect(structures, names(rawList[[1]]$objects)) %>% print()
for(structure in structures) {
  assign(
    structure, 
    rawList %>% 
      map(\(x) x$objects[[structure]] %>% 
            dplyr::select(!any_of(c(#"Number_Object_Number",
              #"Parent_masks_cell",
              "Parent_ConvertImageObjects",
              "Parent_FilterObjects")))) %>% 
      list_rbind(names_to="Metadata_directory") %>% #glimpse()
      right_join(
        dplyr::select(image, starts_with("Metadata"), ImageNumber), ., 
        by=join_by(Metadata_directory, ImageNumber)) %>% 
      ...
      dplyr::rename("{structure}Number" := Number_Object_Number) %>% 
  ...
  all_objects <- c(all_objects, structures) %>% unique()
}
rm(structure, structures)
```

## spot
```{r}
# glimpse(raw$object)
spot_objs<- str_subset(names(rawList[[1]]$objects), "^spot") %>% 
  str_subset("spot_rbg", negate=T)
if(length(spot_objs) > 0) {
  for(spot_obj in spot_objs) {
    assign(
      spot_obj, 
      map(rawList, \(x) x$objects[[spot_obj]] %>% 
        ...
        # rename parent-joining column name
        {if(any(str_detect(colnames(.), "^Parent_"))) {
          dplyr::rename_with(., \(x) str_replace(x, "Parent_", "") %>% str_c("Number"),
                             starts_with("Parent_"))
        } else . } %>%
        dplyr::rename("{spot_obj}Number" := Number_Object_Number) %>%
        # rename probabilities to make plot okay
        rename_with(\(x) str_replace(x, "_Probabilities", "Prob"), 
                    !starts_with("Metadata")) #%>% glimpse()
    )
    
    all_objects <- c(all_objects, spot_obj) %>% unique()
  }
}
rm(spot_obj, spot_objs)
```

## spot_rbg
```{r}
# get spot around background to calculate spot contrast, if has
if("spot_rbg" %in% names(rawList[[1]]$objects)) {
  ...
  spot <- rawList %>% 
    map(\(x) x$objects[["spot_rbg"]] %>% 
          dplyr::rename(spotNumber=Number_Object_Number)) %>% 
    list_rbind(names_to="Metadata_directory") %>% #glimpse()
    dplyr::select(Metadata_directory, ImageNumber, spot_Number, contains("MeanIntensity_")) %>% 
    rename_with(\(x) str_c(x, "rbg", sep=""), 
                !c(Metadata_directory, ImageNumber, spot_Number)) %>% 
    left_join(spot, ., by=join_by(Metadata_directory, ImageNumber, spot_Number)) %>% 
    glimpse()
  
  # calculate spot contrast
  rbg_colnames <- colnames(spot) %>% str_subset("rbg$")
  rbg_prefix <- str_replace_all(rbg_colnames, "rbg", "") %>% unique()
  ...
  for(i in rbg_channels) {
    ...
    spot[str_c(calculate_contrast_by, i, "EnrichFold")] <- 
      round(
        (spot[str_c(calculate_contrast_by, i)] + 1) / (spot[str_c(rbg_prefix, i, "rbg")] + 1), 
        3)
...
if(length(str_subset(ls(), "^spot")) > 0) {
  ...
  for(obj in str_subset(ls(), "^spot")) {
    cell <- get(obj) %>% reframe(
      ...
      .by=c(starts_with("Metadata"), 
            any_of(c("ImageNumber", "cellNumber", "cell_Number")))) %>% 
      left_join(cell, .) %>% 
...
if(F) {
  if(F) {
    ...
    ggplot(cell, aes(Intensity_MeanIntensity_GFP)) +
      geom_histogram(binwidth=20) +
      facet_wrap(vars(Metadata_conc, Metadata_condition), axes="all") +
  ...
  cell_filter <- cell %>% 
    ...
    # filter(between(AreaShape_EquivalentDiameter, 12, 32)) %>%
    # filter(!(str_detect(Metadata_vector, "Kras") & Intensity_MeanIntensity_GFP < 7.5e5)) %>%
    # filter(between(Intensity_MeanIntensity_GFP, 20, 3000)) %>%
    ...
    mutate(Intensity_StdIntensityNormalized_GFP=Intensity_StdIntensity_GFP/
             Intensity_MeanIntensity_GFP) %>% 
    select(!matches("^Parent")) %>% 
    select(!matches("spot.*Total$")) %>% 
    select(!c(starts_with("Children"))) %>%
...
## summarize data
grouping_vars <- c("Metadata_directory", "Metadata_well", "Metadata_group",
                   "Metadata_vector", "Metadata_conc", 
                   "Metadata_temp", "Metadata_batch")
...
pdata <- get(plot_obj) %>% 
  ...
  mutate(Metadata_group=str_c(Metadata_conc, Metadata_condition, sep="_")) %>%
  mutate(Metadata_group=factor(
    Metadata_group, 
    c("0nM_unstim","0nM_stim", "400nM_unstim", "400nM_stim"))) %>%
  # mutate(Metadata_conc=factor(
  #   as.numeric(Metadata_conc), sort(as.numeric(unique(Metadata_conc))))) %>%
...
pdata %>%
  ggplot(aes(Intensity_MeanIntensity_GFP, Intensity_StdIntensity_GFP,
             color=Intensity_MeanIntensity_GFP)) +
  geom_point(size=0.5) +
  facet_wrap(~ Metadata_group, scales="fixed", axes="all") +
  my_theme(0)

p <- pdata %>%
  ggplot(aes(Metadata_group, Intensity_StdIntensityNormalized_GFP)) +
  geom_boxplot(aes(fill=Metadata_group), width=0.5, outliers=F) +
  ggbeeswarm::geom_quasirandom(aes(), width=0.2, color="grey50",
                               size=0.5, alpha=0.5) +
  # facet_wrap(~ Metadata_group, scales="fixed", axes="all") +
  ggpubr::stat_compare_means(comparisons=list(c("0nM_unstim", "0nM_stim"),
                                              c("0nM_unstim", "400nM_unstim"),
                                              c("0nM_unstim", "400nM_stim"),
                                              c("400nM_unstim", "400nM_stim"))) +
  labs(x="") +
  my_theme(0) + theme(legend.position="none")
p
ggsave(str_glue("{Sys.Date()}_{plot_obj}_boxplot.pdf"), p, width=3, height=3)
...
plot_items <- colnames(get(plot_obj)) %>% 
  ...
  str_subset("RadialDistribution", negate=T) %>%
...
plot_list <- plot_items %>% 
  map(\(x) pdata %>% 
        ...
        ggplot(aes(Metadata_group, !!as.name(x), fill=Metadata_group)) +
        ...
        # facet_wrap(vars(Metadata_batch),
...
pdf(str_glue("{Sys.Date()}_{plot_obj}_bar.pdf"), width=2.5, height=2.5)

...
result_db_files <- list.files(
  '.', pattern='results.db', recursive=T, full.names=T) %>% 
...
raw <- names(raw_list[[1]]) %>% 
  ...
  map(\(x) map(raw_list, \(y) y[[x]]) %>% list_rbind(names_to='directory'))
...
# count(pdata, cmpd)
# count(pdata, conc)
# count(pdata, cell, date, cmpd, density)
...
plot_list <- plot_items %>% 
  map(\(x) pdata %>% 
        ...
        stat_summary(geom="bar", fun=mean, color="grey50", linewidth=0.25,
                     ...
                     position=position_dodge(0.75)) + 
        facet_wrap(vars(directory), ncol=1,
...
pdf(str_glue("{Sys.Date()}_{plot_obj}_bar.pdf"), width=12, height=5)
...
p <- pdata %>% 
  ...
  # mutate(metric=metric / mean(metric[conc == 0])) %>% 
  # filter(between(Intensity_MeanIntensity_GFP, 200, 2000)) %>% 
  ggplot(aes(conc, cell_count, fill=date, color=date)) +
  # stat_summary(geom="errorbar", fun.data=mean_sdl,
  #              fun.args=list(mult=1), # check Hmisc::smean.sdl
  #              width=0.4, color="grey20", # "#4575b4"
  #              position=position_dodge(0.75)) +
  ...
  labs(x="Concentration (uM), 3x dilution series") +
  my_theme(angle=0) +
  theme(legend.position="top")
pdf(str_glue("{Sys.Date()}_{plot_obj}_line.pdf"), width=5, height=7)
print(p)
# for(i in plot_list) print(i)
while(dev.cur() != 1) dev.off()

...
plot_items <- colnames(get(plot_obj)) %>% 
  ...
  unique() %>% 
  print()
# plot_items <-c("cell_count", "shape_area")
...
pdata %>% 
  ...
  mutate(metric=cell_count*shape_area) %>% 
  # filter(conc == 0)
  group_by(directory, cmpd) %>% 
  transmute(conc=conc,
            across(metric, ~ .x / mean(.x[conc == 0], na.rm = TRUE))) %>%
  # mutate(metric=metric / mean(metric[conc == 0])) %>% 
  ungroup() %>% 
  # filter(between(Intensity_MeanIntensity_GFP, 200, 2000)) %>% 
  ggplot(aes(conc, metric, fill=cmpd, color=cmpd)) +
  # stat_summary(geom="errorbar", fun.data=mean_sdl,
  #              fun.args=list(mult=1), # check Hmisc::smean.sdl
  #              width=0.4, color="grey20", # "#4575b4"
  #              position=position_dodge(0.75)) +

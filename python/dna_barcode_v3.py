...
def get_strategy_for_length(n: int) -> Dict:
    ...
    if n <= EXHAUSTIVE_MAX_LENGTH:
        ...
        return {
            ...
            'use_khash': True,
            'batch_size': DEFAULT_BATCH_SIZE  # 5000 for better performance
        }
    else:
        return {
            'generation': 'sampling',
...
def validate_params(n: int, k: int, gc_min: float, gc_max: float) -> List[str]:
    ...
    if k > n:
        warnings.append(f"Warning: distance={k} > length={n}, impossible to satisfy")
    
    if gc_min < 0 or gc_min > 1:
        warnings.append(f"Warning: gc_min={gc_min} should be in range [0, 1]")
    
    if gc_max < 0 or gc_max > 1:
        warnings.append(f"Warning: gc_max={gc_max} should be in range [0, 1]")
    
    if gc_min > gc_max:
        warnings.append(f"Warning: gc_min={gc_min} > gc_max={gc_max}, swapping values")
    
    if k < 1:
        warnings.append(f"Warning: distance={k} should be at least 1")
...
def has_homopolymer_int(num: int, length: int, homopolymer: int) -> bool:
    """
    Check for homopolymers using integer representation.
    Converts to sequence first to avoid bit-order confusion.
    """
    if length < homopolymer:
        return False
    
    # Convert to sequence for clarity and correctness
    seq = int_to_seq(num, length)
    
    # Check for consecutive identical nucleotides
    for i in range(length - homopolymer + 1):
        if all(seq[i] == seq[i+j] for j in range(homopolymer)):
            return True
...
def int_to_seq(num: int, length: int) -> str:
    ...
    Least significant 2 bits represent the last nucleotide.
    """
    seq = []
    for _ in range(length):
...
def generate_all_barcodes_exhaustive(n: int, gc_min: float, gc_max: float, 
    ...
    """
    total = 4 ** n
    gc_min_count = int(gc_min * n)
    gc_max_count = int(gc_max * n)
    
    barcodes = []
    
    logger.info(f"Exhaustive generation: checking all {total:,} possible sequences...")
...
def generate_sequences_batch(n: int, gc_min: float, gc_max: float, homopolymer: int,
    ...
    while len(sequences) < target_count and attempts < max_attempts:
        ...
        if (attempts + 1) % 50000 == 0:
            logger.info(f"  Progress: {len(sequences):,}/{target_count:,} found ({100*len(sequences)/max_attempts:.1f}% of max attempts)")
    ...
    for i in range(n):
        for j in (0, 1):
            ...
            if pos < n:
                ...
                if len(kmer) == window:  # Ensure full window
                    arr.append((pos, kmer))
...
def conflicts_with_index(candidate: str, kmer_index: Dict, k: int) -> bool:
    """
    Check if a candidate sequence conflicts with any indexed sequence.
    Uses k-mer hashing to avoid computing all pairwise distances.
    """
    for pos, kmer in khash(candidate, k):
        if (pos, kmer) in kmer_index:
            # Potential conflict - need to check actual distance
            # (k-mer match is necessary but not sufficient for conflict)
            return True
...
def filter_with_khash(candidates: List[str], selected: List[str], k: int) -> List[str]:
    ...
    for candidate in candidates:
        # Quick k-mer check
        if not conflicts_with_index(candidate, kmer_index, k):
            result.append(candidate)
        else:
            # Verify with actual distance check (k-mer match may be false positive)
            valid = True
            for sel in selected:
                d = levenshtein_distance_cutoff(candidate, sel, k - 1)
                if d < k:
...
def create_barcode_set_target(n: int, k: int, gc_min: float, gc_max: float,
    ...
    if strategy['generation'] == 'exhaustive':
        ...
        batch_size = strategy.get('batch_size', DEFAULT_BATCH_SIZE)
        ...
        for round_idx in range(max_rounds):
            ...
            # Adaptive batch size: larger if yield is low, but cap at 10000 for performance
            if round_idx > 0:
                expected_yield = max(1, int(batch_size * 0.1))  # Assume 10% yield
                batch_size = min(batch_size, remaining * 5, 10000)  # Cap at 10000
            ...
            if len(candidates) > 1:
                ...
                candidates = greedy_filter_batch(candidates, k, max_output=max_internal)
...
def greedy_filter_batch(candidates: List[str], k: int, max_output: int = None) -> List[str]:
    """
...
def sparse_view(xs: List[str], D: Dict, symmetric: bool = True):
    ...
    if len(D) == 0:
        ...
        i, j, data = zip(*[(f(a), f(b), True) for (a, b), v in D.items()])
        i = np.array(i)
        j = np.array(j)
        data = np.array(data)
...
def validate_barcode_set(barcodes: List[str], k: int, 
    ...
    for i in range(len(barcodes)):
        for j in range(i + 1, len(barcodes)):
            d = levenshtein_distance_cutoff(barcodes[i], barcodes[j], k - 1)
            checked += 1
            
            if d < k:
                failures.append((barcodes[i], barcodes[j], d))
...
def main():
    ...
    logging.basicConfig(
        ...
        datefmt='%Y-%m-%d %H:%M:%S',
        level=logging.INFO  # Fixed to INFO for better debugging
    )

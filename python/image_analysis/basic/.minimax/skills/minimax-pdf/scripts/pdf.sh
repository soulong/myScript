...
cmd_fix() {
  ...
  if command -v npm &>/dev/null; then
    ...
    # Install pdf-lib for --preserve-links support (hyperlink injection into PDF)
    echo "Installing pdf-lib (for hyperlink preservation)..."
    if [[ -f "$SCRIPT_DIR/package.json" ]]; then
      (cd "$SCRIPT_DIR" && npm install --omit=dev 2>/dev/null) || echo "Warning: pdf-lib install failed (links may not be clickable)"
    fi
  else
...
cmd_html() {
  ...
  append_node_path
  # --preserve-links 需要 pdf-lib，若本地 node_modules 不存在则按需安装（npm 缓存已预热，秒级完成）
  if [[ "$*" == *"--preserve-links"* ]] && [[ -f "$SCRIPT_DIR/package.json" ]] && [[ ! -d "$SCRIPT_DIR/node_modules" ]]; then
    echo "Installing npm dependencies for link preservation..."
    (cd "$SCRIPT_DIR" && npm install --omit=dev 2>/dev/null) || echo "Warning: npm install failed, links may not be clickable"
  fi

...
# Searches multiple locations: PLAYWRIGHT_BROWSERS_PATH, $HOME/.cache, /root/.cache, etc.
get_installed_chromium_revision() {
    local dirs=()

    # 1. PLAYWRIGHT_BROWSERS_PATH (highest priority, set in Dockerfile)
    if [[ -n "${PLAYWRIGHT_BROWSERS_PATH:-}" ]] && [[ -d "$PLAYWRIGHT_BROWSERS_PATH" ]]; then
        dirs+=("$PLAYWRIGHT_BROWSERS_PATH")
    fi

    # 2. Default cache for current user
    case "$(uname -s)" in
        Darwin) dirs+=("$HOME/Library/Caches/ms-playwright") ;;
        *)      dirs+=("$HOME/.cache/ms-playwright") ;;
    esac

    # 3. Common system locations (Docker/container environments)
    dirs+=("/root/.cache/ms-playwright")
    dirs+=("/home/minimax/.cache/ms-playwright")
    dirs+=("/opt/playwright-browsers")

    # Search all directories, return highest revision found
    local best_revision=""
    for cache in "${dirs[@]}"; do
        ...
        if [[ -n "$revision" ]]; then
            if [[ -z "$best_revision" ]] || (( revision > best_revision )); then

...
const { generateLinkMap, injectLinksIntoPdf, saveLinkMap } = require('./link_injector');
...
Options:
  ...
  --preserve-links      Preserve hyperlinks in PDF (requires pdf-lib)
...
Examples:
  ...
  node html_to_pdf.js document.html --preserve-links
    `);
    process.exit(0);
  }
  ...
  for (let i = 1; i < args.length; i++) {
    if (args[i] === '--output' || args[i] === '-o') {
      ...
      customCSS = args[++i];
    } else if (args[i] === '--preserve-links') {
      preserveLinks = true;
    } else if (args[i] === '--save-link-map') {
      saveLinkMapFlag = true;
    }
  }
  ...
  return { inputFile, outputFile, customCSS, preserveLinks, saveLinkMapFlag };
}
...
// Main function
async function convertHTMLToPDF(inputFile, outputFile, customCSS, preserveLinks = false, saveLinkMapFlag = false) {
  ...
  try {
    ...
    // Inline external local CSS files before Paged.js injection
    // Paged.js re-fetches CSS to parse @page margin boxes, but file:// cross-directory
    // fetches may be blocked by Chromium security policy, causing pagination to hang.
    // Solution: read local CSS files and inject them as inline <style> tags.
    const inlinedCount = await page.evaluate(() => {
      const links = document.querySelectorAll('link[rel="stylesheet"]');
      let count = 0;
      links.forEach(link => {
        const href = link.getAttribute('href') || '';
        // Only inline local file paths (absolute paths starting with /)
        if (href.startsWith('/') && !href.startsWith('//') && !href.startsWith('http')) {
          // Mark for server-side inlining
          link.setAttribute('data-inline-path', href);
          count++;
        }
      });
      return count;
    ...
    if (inlinedCount > 0) {
      const linksToInline = await page.evaluate(() => {
        return Array.from(document.querySelectorAll('link[data-inline-path]'))
          .map(link => link.getAttribute('data-inline-path'));
      });

      for (const cssPath of linksToInline) {
        try {
          if (fs.existsSync(cssPath)) {
            const cssContent = fs.readFileSync(cssPath, 'utf-8');
            await page.evaluate(({ path: p, css }) => {
              const link = document.querySelector(`link[data-inline-path="${p}"]`);
              if (link) {
                const style = document.createElement('style');
                style.textContent = css;
                style.setAttribute('data-inlined-from', p);
                link.parentNode.replaceChild(style, link);
              }
            }, { path: cssPath, css: cssContent });
            console.log(`  Inlined CSS: ${cssPath}`);
          } else {
            console.log(`  Warning: CSS file not found: ${cssPath}`);
          }
        } catch (e) {
          console.log(`  Warning: Failed to inline CSS ${cssPath}: ${e.message}`);
        }
      }
    }
    ...
    // Extract link map if preserveLinks is enabled
    let linkMap = null;
    if (preserveLinks || saveLinkMapFlag) {
      console.log('Extracting hyperlink positions...');
      linkMap = await generateLinkMap(page);
      console.log(`  Found ${linkMap.stats.total} links (${linkMap.stats.internal} internal, ${linkMap.stats.external} external)`);
    }
    ...
    // Inject links into PDF if enabled
    if (linkMap && preserveLinks) {
      console.log('Injecting hyperlinks into PDF...');
      try {
        const injectedCount = await injectLinksIntoPdf(outputPath, linkMap);
        if (injectedCount > 0) {
          console.log(`  ✓ Injected ${injectedCount} clickable links`);
        }
      } catch (err) {
        console.warn(`  ⚠️  Link injection failed: ${err.message}`);
      }
    }

    // Save link map to JSON if requested
    if (linkMap && saveLinkMapFlag) {
      const jsonPath = saveLinkMap(outputPath, linkMap);
      console.log(`  Link map saved to: ${path.basename(jsonPath)}`);
    }
...
(async () => {
  try {
    const { inputFile, outputFile, customCSS, preserveLinks, saveLinkMapFlag } = parseArgs();
    await convertHTMLToPDF(inputFile, outputFile, customCSS, preserveLinks, saveLinkMapFlag);
  } catch (error) {

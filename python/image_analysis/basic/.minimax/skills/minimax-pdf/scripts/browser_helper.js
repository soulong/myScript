...
function loadPlaywright() {
  try {
    ...
    // On Windows, add common global npm paths directly (faster than npm root -g)
    if (process.platform === 'win32') {
      const appData = process.env.APPDATA;
      if (appData) {
        searchRoots.add(path.join(appData, 'npm', 'node_modules'));
      }
      const localAppData = process.env.LOCALAPPDATA;
      if (localAppData) {
        searchRoots.add(path.join(localAppData, 'npm', 'node_modules'));
      }
      // nvm-windows / fnm / volta common paths
      const userProfile = process.env.USERPROFILE || os.homedir();
      if (userProfile) {
        searchRoots.add(path.join(userProfile, 'AppData', 'Roaming', 'npm', 'node_modules'));
        searchRoots.add(path.join(userProfile, '.nvm', 'versions', 'node', `v${process.version.slice(1)}`, 'lib', 'node_modules'));
      }
      // Program Files paths
      const programFiles = process.env.ProgramFiles || 'C:\\Program Files';
      searchRoots.add(path.join(programFiles, 'nodejs', 'node_modules'));
    }

    try {
      const npmRoot = execSync('npm root -g', {
        stdio: ['ignore', 'pipe', 'ignore'],
        timeout: 10000  // 10s timeout to prevent hanging on Windows
      }).toString().trim();
      if (npmRoot) {
        searchRoots.add(npmRoot);
      }
    } catch (_) {
      // ignore npm errors or timeout
    }
    ...
    // Provide actionable error message to guide the agent to fix (not degrade)
    const searchedPaths = Array.from(searchRoots).filter(Boolean).join('\n    ');
    throw new Error(
      'Playwright module not found.\n' +
      '\n' +
      '⚠️ FIX (run these commands, then retry the same conversion command):\n' +
      '  npm install -g playwright && npx playwright install chromium\n' +
      '\n' +
      '⛔ DO NOT fall back to convert_file or any other tool.\n' +
      '   convert_file produces low-quality screenshot-based PDFs.\n' +
      '   You MUST fix the dependency and retry pdf.sh / html_to_pdf.js.\n' +
      '\n' +
      'Searched paths:\n    ' + searchedPaths
    );
  }

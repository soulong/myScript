...
sc_file = os.path.abspath(sys.argv[2])
...
def main():
    ...
    # df = pd.read_table(sc_file, sep="\s+")
    df = pd.read_csv(sc_file, sep='\s+', comment='#')
    # print(df)
    print(df.columns)
    ...
    df = df.sort_values(by=['pae_interaction'])

    df[['description','sequence','length','pae_interaction','pae_binder','binder_aligned_rmsd','plddt_binder','pae_target','target_aligned_rmsd','plddt_target']].to_csv(out_file, index=False)

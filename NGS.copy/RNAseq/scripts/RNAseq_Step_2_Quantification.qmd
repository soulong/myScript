...
params:
  ...
  # if get quantification from bam file
  # if None, infer from salmon log file
  ...
  bam_dir: "bam"
  featurecounts_path: "~/miniconda3/envs/ngs/bin/featureCounts"
  lib_type_featurecounts: "None"
  thread: 30
  hs_gtf_file: "/media/hao/Data1/index/hs/GRCh38.gtf.gz"
  mm_gtf_file: "/media/hao/Data1/index/mm/GRCm38.gtf.gz"
---
...
if(params$quant_from_bam) {
  if(!(list.files(params$quantification_dir) %>%
    ...
    cat(str_c("log file can be found at: ", 
              file.path(params$quantification_dir, "featurcounts.log\n\n")))
    
    if(!dir.exists(params$quantification_dir))
      dir.create(params$quantification_dir)
    
    if(file.exists(
      file.path(params$quantification_dir, 'gene_featurecounts.txt'))) {
      ...
      system2(featurecounts_path, args = featurcounts_args)
      toc()
    }
    
    
    ####### add annotation info to featurecounts output
    cat("\nfeaturcounts gene annoation\n")
    gene_count_featurecounts <- suppressMessages(
      read_table2(paste0(params$quantification_dir, "/gene_featurecounts.txt"), 
                  comment = "#")) %>%

---
title: "CellProfiler Analysis for Compound Screening"
format: html
editor_options: 
...
#   dirname() |>
#   setwd()
setwd("D:\\Postdoc_Data\\2024-02-29_TetOff_titer_48h")
...
if(file.exists(plate_info_f)) {
  plate_info <- plate_info_f %>%
    ...
    mutate(Metadata_vector = factor(Metadata_vector,
                                   levels=rev(unique(Metadata_vector))),
           Metadata_vector2 = factor(Metadata_vector2,
                                   levels=unique(Metadata_vector2)))
...
# Image <- filter(Image, is.na(Metadata_adaptor), !is.na(Metadata_vector))
```
...
fill_var <- "Metadata_vector2"
fact_var <- NA # NA mean no additional facet beside channel

intensity_category <- "Percentile_99" # MeanIntensity


for(obj in objects) {
  
  channels <- colnames(Image) %>% 
    str_subset(str_glue("{obj}_Intensity_{intensity_category}_.*")) %>% 
    str_replace_all(str_glue("{obj}_Intensity_{intensity_category}_"), "")
  
  intensity_data <- eval(parse(text=obj)) %>% 
    pivot_longer(
      any_of(as.character(str_glue(
        "{obj}_Intensity_{intensity_category}_{channels}"))),
      names_to = "channel", 
      names_prefix = str_glue("{obj}_Intensity_{intensity_category}_"),
      values_to = "Intensity"
    )
  ...
  ggsave2(str_glue("{Sys.Date()}_{obj}_intensity_{intensity_category}_log2.pdf"),
          p,
          width=5, 
          height=4)
}

...
#   dirname() |>
#   setwd()
setwd("D:\\Postdoc_Data\\Project_dRTK\\2024-03-09_dRTK_GRB2-SH2-SHC1_serum")
...
if(file.exists(plate_info_f)) {
  plate_info <- plate_info_f %>%
    ...
    mutate(Metadata_vector = factor(Metadata_vector,
                                   levels=sort(unique(Metadata_vector))),
...
# glimpse(raw$objects)
cell <- raw$objects[[1]] %>% 
  select(!any_of(c("Number_Object_Number",
...
group_vars <- c("Metadata_prefix", "Metadata_vector", "Metadata_serum")
...
ggsave2(str_glue("{Sys.Date()}_{obj}_count_per_field_point.pdf"),
        p,
        width=length(unique(count_per_field_per_well[[axis_var]]))*0.1, 
        height=ifelse(
          is.na(fact_var), 
          2,
...
ggsave2(str_glue("{Sys.Date()}_{obj}_count_per_field_bar.pdf"),
        p,
        width=length(unique(count_per_field_per_well[[axis_var]])) * 0.2, 
        height=ifelse(
          is.na(fact_var), 
          2,
          2*length(unique(count_per_field_per_well[[fact_var]]))),
...
# ggsave2(str_glue("{Sys.Date()}_{obj}_count_per_field_box.pdf"),
#         p,
#         width=length(unique(count_per_field_per_well[[axis_var]]))*0.1, 
#         height=ifelse(
#           is.na(fact_var), 
...
intensity_data_per_well <- intensity_data %>% 
  reframe(Intensity_mean_per_well = mean(Intensity, trim = 0.1),
          .by = all_of(
            c(group_vars, "Metadata_well", "Metadata_field", "channel")))

# boxplot
p <- intensity_data_per_well %>% 
  ggplot(aes(!!as.name(axis_var),
             Intensity_mean_per_well,
             fill = !!as.name(fill_var))) +
  geom_boxplot(position = position_dodge(0.9, preserve = "single"),
               linewidth=0.25,
  ...
  labs(y="Mean Intensity per Well", x="") +
  # coord_cartesian(xlim = c(0, 200), ylim = c(0, 2000)) +
  theme_bw(8) +
  theme(strip.background = element_rect(fill=NA), 
        strip.text = element_text(size=6),
        ...
        legend.title = element_blank(),
        legend.key.size = unit(1.5, "mm"))
ggsave2(str_glue("{Sys.Date()}_{obj}_intensity_per_well_box.pdf"),
        p,
        width=length(unique(eval(parse(text=obj))[[axis_var]]))*0.1*
          length(unique(intensity_data$channel)), 
        height=ifelse(
